...
function make_username($first_name,$last_name,$table) {
...
function mailDriverVerificationTemplate($user) {
    $data = [
        'driver_id'     => $user->id,
        'code'          => generate_random_code(),
        'token'         => generate_unique_string("driver_authorizations","token",200),
        'created_at'    => now(),
    ];

    DB::beginTransaction();
    try{
        DriverAuthorization::where("driver_id",$user->id)->delete();
        DB::table("driver_authorizations")->insert($data);
        try{
            $user->notify(new SendAuthorizationCode((object) $data));
        }catch(Exception $e){}
        DB::commit();
    ...
    return redirect()->route('driver.authorize.mail',$data['token'])->with(['warning' => ['Please verify your mail address. Check your mail inbox to get verification code']]);
}
...
//driver two factor template
function google_two_factor_verification_driver_template($user) {
    return redirect()->route('driver.authorize.google.2fa')->with(['error' => ['Please verify two factor authentication']]);
}
...
// qrcode for driver
function generate_google_2fa_auth_driver_qr() {
    $google2FA = new \PragmaRX\Google2FA\Google2FA();
    $secret_key = $google2FA->generateSecretKey();
