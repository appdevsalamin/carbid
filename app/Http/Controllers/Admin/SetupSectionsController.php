...
class SetupSectionsController extends Controller
{
    ...
    public function section($slug,$type) {
        $sections = [
            ...
            'about-us'  => [
                ...
                'ItemStatusUpdate'   => "aboutUsStatusUpdate",
            ],
            'services'  => [
                'view'          => "servicesView",
            ...
            'clients-feedback' => [
                ...
                'ItemStatusUpdate'   => "clientFeedBackStatusUpdate",
            ],
            'announcement' => [
            ...
            'how-it-work' => [
                ...
                'ItemStatusUpdate'   => "howItWorkStatusUpdate",
            ],
            'about-page'  => [
                'view'          => "aboutPageView",
            ...
            'statistics' => [
                ...
                'ItemStatusUpdate'   => "statisctisStatusUpdate",
            ],
            'whychoose' => [
                'view'              => "whyChooseView",
                ...
                'ItemStatusUpdate'  => "whyChooseStatusUpdate",
            ],
            'login' => [
                'view'          => "loginView",
                'update'        => "loginUpdate",
     ...
     */
    public function sectionStatusUpdate(Request $request,$slug){
        $section = $this->section($slug,'ItemStatusUpdate');
        return $this->$section($request,$slug);
    }
    ...
    public function bannerUpdate(Request $request,$slug) {
       ...
       // Background image
        ...
        // Left image
        $data['image1'] = $section->value->image1 ?? null;
        if ($request->hasFile("image1")) {
            $data['image1'] = $this->imageValidate($request, "image1", $section->value->image1 ?? null);
        }

        // Right image
        $data['image2'] = $section->value->image2 ?? null;
        if ($request->hasFile("image2")) {
            $data['image2'] = $this->imageValidate($request, "image2", $section->value->image2 ?? null);
        }
    ...
    /**
     * About us status update
     */
    public function aboutUsStatusUpdate(Request $request,$slug)
    {
        $validator = Validator::make($request->all(),[
    ...
    public function servicesUpdate(Request $request,$slug) {
        ...
        if($section != null) {
         ...
         $section_data['image'] = $section->value->image ?? null;
            if ($request->hasFile("image")) {
                $section_data['image'] = $this->imageValidate(
                    $request,
                    "image",
                    $section->value->image ?? null
                );
            }
    ...
    // Set default background image value safely
    $data['background_image'] = $section->value->background_image ?? "";

    // Validate image
    $validator = Validator::make($request->all(), [
        'background_image' => "nullable|image|mimes:jpg,png,svg,webp|max:10240",
    ]);
    ...
    if ($request->hasFile("background_image")) {
        $data['background_image'] = $this->imageValidate($request, "background_image", $data['background_image']);
    }
    ...
    // Add validated language fields
    $section_data['language'] = $this->contentValidate($request, $basic_field_name);

    // Add updated background image to section_data
    $section_data['background_image'] = $data['background_image'];
     ...
     * 
     */
    public function clientFeedBackStatusUpdate(Request $request,$slug)
    {
         $validator = Validator::make($request->all(),[
            'status'                    => 'required|boolean',
            'data_target'               => 'required|string',
    ...
    public function howItWorkItemStore(Request $request,$slug) {
        ...
        $language_wise_data = $this->contentValidate($request,$basic_field_name,"how-work-item-add");
        if($language_wise_data instanceof RedirectResponse) return $language_wise_data;
        $slug = Str::slug(SiteSectionConst::HOW_IT_WORK_SECTION);
        $section = SiteSections::where("key",$slug)->first();
    ...
    public function howItWorkItemUpdate(Request $request,$slug) {
        ...
        $language_wise_data = $this->contentValidate($request,$basic_field_name,"how-page-item-edit");
        if($language_wise_data instanceof RedirectResponse) return $language_wise_data;
    ...
    public function footerUpdate(Request $request,$slug) {
        ...
        $slugKey = Str::slug(SiteSectionConst::FOOTER_SECTION);
        $section = SiteSections::where('key', $slugKey)->first();

        // Keep old image if exists
        $data['background_image'] = isset($section->value->background_image) ? $section->value->background_image : "";


        // Replace if a new image is uploaded
            if ($request->hasFile("background_image")) {
            $data['background_image'] = $this->imageValidate(
                $request,
                "background_image",
                $section->value->background_image ?? null
            );
        }

        // Update or create section
        try {
            SiteSections::updateOrCreate(
                ['key' => $slugKey],
                [
                    'key'   => $slugKey,
                    'value' => $data,
    ...
    /**
     * Method for delete section background image
     * @param string $slug
     * @param \Illuminate\Http\Request  $request 
     */
    public function statisticsView($slug) {
        $page_title = "Statistics Page Section";
        $section_slug = Str::slug(SiteSectionConst::STATISTICTS_SECTION);
        $data = SiteSections::getData($section_slug)->first();
        $languages = $this->languages;

        return view('admin.sections.setup-sections.statistics-page-section',compact(
            'page_title',
            'data',
            'languages',
            'slug',
    ...
    public function statisticsUpdate(Request $request,$slug) {
        ...
        $slug = Str::slug(SiteSectionConst::STATISTICTS_SECTION);
        $section = SiteSections::where("key",$slug)->first();
    ...
    public function statisticsItemStore(Request $request,$slug) {
        ...
        $language_wise_data = $this->contentValidate($request,$basic_field_name,"statisticts-page-item-add");
        if($language_wise_data instanceof RedirectResponse) return $language_wise_data;
        $slug = Str::slug(SiteSectionConst::STATISTICTS_SECTION);
        $section = SiteSections::where("key",$slug)->first();
    ...
    public function statisticsItemUpdate(Request $request,$slug) {
        ...
        $language_wise_data = $this->contentValidate($request,$basic_field_name,"statisticts-page-item-edit");
        if($language_wise_data instanceof RedirectResponse) return $language_wise_data;
    ...
    /**
     * Update the statisticsts status
     * 
     */
    public function statisctisStatusUpdate(Request $request,$slug)
    {
         $validator = Validator::make($request->all(),[
            'status'                    => 'required|boolean',
            'data_target'               => 'required|string',
    ...
    public function whyChooseItemStore(Request $request,$slug) 
    {
        ...
        if($validator->fails()) return back()->withErrors($validator)->withInput()->with('modal','why-choose-add');
        $validated = $validator->validate();

        $language_wise_data = $this->contentValidate($request,$basic_field_name,"why-choose-add");
        if($language_wise_data instanceof RedirectResponse) return $language_wise_data;
        $slug = Str::slug(SiteSectionConst::WHY_CHOOSE_SECTION);
        $section = SiteSections::where("key",$slug)->first();
    ...
    /**
     * Why choose status update
     * 
     */
    public function whyChooseStatusUpdate(Request $request,$slug) 
    {
         $validator = Validator::make($request->all(),[
            'status'                    => 'required|boolean',
            'data_target'               => 'required|string',
    ...
    public function registerView($slug)
    {
        ...
        $section_slug = Str::slug(SiteSectionConst::REGISTRATION_SECTION);
        $data = SiteSections::getData($section_slug)->first();
        $languages = $this->languages;
    ...
    public function registerUpdate(Request $request,$slug)
    {
        ...
        $slug = Str::slug(SiteSectionConst::REGISTRATION_SECTION);
        $section = SiteSections::where("key",$slug)->first();
