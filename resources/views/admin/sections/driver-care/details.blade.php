...
@section('content')
    ...
    <div class="custom-card mt-15">
        <div class="card-header">
            <h6 class="title">{{ __("Driver Overview") }}</h6>
        </div>
        <div class="card-body">
                ...
                <div class="row align-items-center mb-10-none">
                        ...
                        <div class="user-action-btn-area">
                            ...
                            <div class="user-action-btn">
                                @include('admin.components.link.custom',[
                                    'href'          => setRoute('admin.drivers.login.logs',$driver->username),
                                    'class'         => "bg--base two",
                                    'icon'          => "las la-sign-in-alt me-1",
                                    'text'          => "Login Logs",
                                    'permission'    => "admin.drivers.login.logs",
                                ])
                            </div>
                            <div class="user-action-btn">
                                @include('admin.components.link.custom',[
                                    ...
                                    'text'          => "Send Email",
                                    'permission'    => "admin.drivers.send.mail",
                                ])
                            </div>
                            <div class="user-action-btn">
                                @include('admin.components.link.custom',[
                                    ...
                                    'permission'    => "admin.drivers.login.as.member",
                                ])
                            </div>
                            <div class="user-action-btn">
                                @include('admin.components.link.custom',[
                                    'href'          => setRoute('admin.drivers.mail.logs',$driver->username),
                                    'class'         => "bg--base five",
                                    'icon'          => "las la-history me-1",
                                    'text'          => "Email Logs",
                                    'permission'    => "admin.drivers.mail.logs",
                                ])
                            </div>
                        </div>
                    </div>
                    <div class="col-xl-4 col-lg-4 form-group">
                        <div class="user-profile-thumb">
                            <img src="{{ $driver->userImage }}" alt="user">
                        </div>
                    </div>
                    <div class="col-xl-4 col-lg-4 form-group">
                        <ul class="user-profile-list">
                            <li class="bg--base one">Full Name: <span>{{ $driver->fullname }}</span></li>
                            <li class="bg--info two">Username: <span>{{ "@".$driver->username }}</span></li>
                            <li class="bg--success three">Email: <span>{{ $driver->email }}</span></li>
                            <li class="bg--warning four">Status: <span>{{ $driver->stringStatus->value }}</span></li>
                            <li class="bg--danger five">Last Login: <span>{{ $driver->lastLogin }}</span></li>
                        </ul>
                    </div>
                </div>
            </form>
    ...
    <div class="custom-card mt-15">
        <div class="card-header">
            <h6 class="title">{{ __("Information of Driver") }}</h6>
        </div>
        <div class="card-body">
            <form class="card-form" method="POST" action="{{ setRoute('admin.drivers.details.update',$driver->username) }}">
                @csrf
                <div class="row mb-10-none">
                    ...
                    <div class="col-xl-6 col-lg-6 form-group">
                        ...
                        <div class="input-group">
                            <div class="input-group-text phone-code">+{{ $driver->mobile_code }}</div>
                            <input class="phone-code" type="hidden" name="mobile_code" value="{{ $driver->mobile_code }}" />
                            <input type="text" class="form--control" placeholder="Write Here..." name="mobile" value="{{ old('mobile',$driver->mobile) }}">
                        </div>
                        @error("mobile")
                    ...
                    <div class="col-xl-6 col-lg-6 form-group">
                        @include('admin.components.form.input',[
                            'label'         => "Address",
                            'name'          => 'address',
                            'value'         => old("address",$driver->address->address ?? ""),
                            'placeholder'   => "Write Here...",
                        ])
                    </div>
                    <div class="col-xl-6 col-lg-6 form-group">
                        <label>{{ __("Country") }}</label>
                        <select name="country" class="form--control select2-auto-tokenize country-select" data-placeholder="Select Country" data-old="{{ old('country',$driver->address->country ?? "") }}"></select>
                    </div>
                    <div class="col-xl-6 col-lg-6 form-group">
                        @php
                            $old_state = old('state',$driver->address->state ?? "");
                        @endphp
                    ...
                    <div class="col-xl-6 col-lg-6 form-group">
                        @php
                            $old_city = old('city',$driver->address->city ?? "");
                        @endphp
                        <label>{{ __("City") }}</label>
                    ...
                    <div class="col-xl-6 col-lg-6 form-group">
                        @include('admin.components.form.input',[
                            ...
                            'name'          => "zip_code",
                            'placeholder'   => "Write Here...",
                            'value'         => old('zip_code',$driver->address->zip ?? "")
                        ])
                    </div>
                    <div class="col-xl-3 col-lg-3 col-md-6 col-sm-6 form-group">
                        @include('admin.components.form.switcher', [
                            'label'         => 'User Status',
                            'value'         => old('status',$driver->status),
                            'name'          => "status",
                            'options'       => ['Active' => 1, 'Banned' => 0],
                            'permission'    => "admin.drivers.details.update",
                        ])
                    </div>
                    <div class="col-xl-3 col-lg-3 col-md-6 col-sm-6 form-group">
                        @include('admin.components.form.switcher', [
                            'label'         => 'Email Verification',
                            'value'         => old('email_verified',$driver->email_verified),
                            'name'          => "email_verified",
                    ...
                    <div class="col-xl-3 col-lg-3 col-md-6 col-sm-6 form-group">
                        @include('admin.components.form.switcher', [
                            ...
                            'value'     => old('two_factor_verified',$driver->two_factor_verified),
                    ...
                    <div class="col-xl-3 col-lg-3 col-md-6 col-sm-6 form-group">
                        @include('admin.components.form.switcher', [
                            ...
                            'value'     => old('kyc_verified',$driver->kyc_verified),
                            'name'      => "kyc_verified",
                            'options'   => ['Verified' => 1, 'Unverified' => 0],
                            'permission'    => "admin.drivers.details.update",
                        ])
    ...
    {{-- Send Email Modal --}}
    @include('admin.components.modals.send-mail-driver',compact("driver"))
@endsection
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $(".login-as-member").click(function() {
                var action  = "{{ setRoute('admin.drivers.login.as.member',$driver->username) }}";
                var target  = "{{ $driver->username }}";
