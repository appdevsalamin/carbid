@if ($basic_settings->driver_kyc_verification == true && isset($kyc_data) && $kyc_data != null && $kyc_data->fields != null)

    <div class="custom-card mt-30">
        <div class="dashboard-header-wrapper">
            <h4 class="title">{{ __("KYC Information") }} &nbsp; <span class="{{ auth('driver_gurd')->user()->kycStringStatus->class }}">{{ auth('driver_gurd')->user()->kycStringStatus->value }}</span></h4>
        </div>
        <div class="card-body">
            @if (auth('driver_gurd')->user()->kyc_verified == global_const()::PENDING)
            ...
            @elseif (auth('driver_gurd')->user()->kyc_verified == global_const()::APPROVED)
                <div class="approved text--success kyc-text">{{__("Your KYC information is verified")}}</div>
                <ul class="kyc-data">
                    @foreach (auth('driver_gurd')->user()->kyc->data ?? [] as $item)
                        <li>
                            @if ($item->type == "file")
                                @php
            ...
            @elseif (auth('driver_gurd')->user()->kyc_verified == global_const()::REJECTED)
                <div class="unverified text--danger kyc-text d-flex align-items-center justify-content-between mb-4">
                    ...
                    <a href="{{ setRoute('driver.authorize.kyc') }}" class="btn--base">{{ __("Verify KYC") }}</a>
                </div>
                <div class="rejected">
                    <div class="rejected-reason">{{ auth('driver_gurd')->user()->kyc->reject_reason ?? "" }}</div>
                </div>
            @else
