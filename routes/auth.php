...
use App\Http\Controllers\Driver\Auth\ForgotPasswordController as DriverForgotPasswordController;
use App\Http\Controllers\Driver\Auth\LoginController as DriverLoginController;
use App\Http\Controllers\Driver\Auth\RegisterController as DriverRegisterController;
use App\Http\Controllers\Driver\Auth\SocialAuthentication as DriverSocialAuthentication;
use App\Http\Controllers\Driver\AuthorizationController as DriverAuthorizationController; 
...
Route::name('user.')->group(function(){
    ...
    Route::controller(AuthorizationController::class)->prefix("authorize")->name('authorize.')->middleware("auth")->group(function(){
        ...
        Route::get('mail/resend/{token}', 'resendMail')->name('mail.resend');
...
Route::prefix("driver")->name("driver.")->group(function () {
    ...
    Route::get('register/{refer?}',[DriverRegisterController::class,"showRegistrationForm"])->name('register');
    Route::post('register',[DriverRegisterController::class,"register"])->name('register.submit');

    Route::controller(DriverForgotPasswordController::class)->prefix("password")->name("password.")->group(function(){
        Route::get('forgot','showForgotForm')->name('forgot');
        Route::post('forgot/send/code','sendCode')->name('forgot.send.code');
        Route::get('forgot/code/verify/form/{token}','showVerifyForm')->name('forgot.code.verify.form');
        Route::post('forgot/verify/{token}','verifyCode')->name('forgot.verify.code');
    ...
    Route::controller(DriverSocialAuthentication::class)->prefix('oauth')->name('social.auth.')->group(function(){
        Route::get('google','google')->name('google');
        Route::get('google/response','googleResponse');
        Route::get('facebook','facebook')->name('facebook');
        Route::get('facebook/response','facebookResponse');
    ...
    Route::controller(DriverAuthorizationController::class)->prefix("authorize")->name('authorize.')->middleware("auth:driver_gurd")->group(function(){
        Route::get('mail/{token}','showMailFrom')->name('mail');
        Route::post('mail/verify/{token}','mailVerify')->name('mail.verify');
        Route::get('mail/resend/{token}', 'resendMail')->name('mail.resend');
        Route::get('kyc','showKycFrom')->name('kyc');
